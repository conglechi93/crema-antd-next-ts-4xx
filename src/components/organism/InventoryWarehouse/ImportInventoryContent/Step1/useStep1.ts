...
import {onUploadInventoriesFile} from 'redux/actions/Inventory';
...
const useStep1 = (
  ...
  const handleExportExcel = async (): Promise<void> => {
    ...
    const formCode = dataSource?.form?.code;
    if (!formCode) return;
    ...
    if (res) {
      ...
      link.href = url;
      link.setAttribute('download', `${dataSource?.name}.xlsx`);
  ...
  const handleCustomRequest = async (options: any) => {
    ...
    const {draftString} = info;
    const dataSource = loadState(draftString);
    const warehouseCode = dataSource?.code;
    const res: any = await dispatch(
      onUploadInventoriesFile(file, warehouseCode),
    );
    const {code} = res;
    if (code) {
