...
const useCustomerDetailFilterTab3 = (
  ...
  useEffect(() => {
    if (categories) {
      ...
      categories?.customerTypeLogShopCat?.map((item: any) => {
        typeOptions.push({
          label: item.name,
          value: item.code,
        });
  ...
  const handleSearch = () => {
    const {time, types} = form.getFieldsValue();
    const searchParams = {
      page: 1,
...
export default useCustomerDetailFilterTab3;
