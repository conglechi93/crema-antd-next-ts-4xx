...
const TaskAddAndUpdate = (props: PropsTypes) => {
  ...
  const handleCustomRequest = async (options: any) => {
    ...
    if (res) {
      ...
      setFileAttachments(newFileList);
      onSuccess('ok');
    } else {
      onError('error');
    }
      ...
      <Row gutter={[16, 16]}>
        <Col xs={16}>
          <Row gutter={[16, 16]}>
            ...
            <AppFormItem name='fileAttachments' label=''></AppFormItem>
            <Col xs={24}>
        ...
        <Col xs={8}>
          <WorkDetail form={form} record={record} info={info} />
        </Col>
      </Row>
    </AppForm>
  );
...
export default memo(TaskAddAndUpdate); // TaskAddAndUpdate;
