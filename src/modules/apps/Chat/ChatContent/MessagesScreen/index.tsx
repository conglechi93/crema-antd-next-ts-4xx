...
const MessagesScreen: React.FC<MessagesScreenProps> = ({
  ...
  const sendFileMessage = (fileMessage: MessageDataObjType) => {
    ...
    postDataApi<ChatApiResponseType>('chat', infoViewActionsContext, {
      channelId: selectedUser?.channelId,
      message: data,
    })
      .then((data) => {
  ...
  const onSend = (message: string) => {
    ...
    if (isEdit) {
      data.edited = true;
      putDataApi<ChatApiResponseType>('chat', infoViewActionsContext, {
        channelId: selectedUser?.channelId,
        message: data,
      })
        .then((data) => {
  ...
  const clearChatHistory = () => {
    postDataApi<ChatApiResponseType>('chat/clearChat', infoViewActionsContext, {
      channelId: selectedUser?.channelId,
    })
      .then((data) => {
        setData(data?.userMessages);
